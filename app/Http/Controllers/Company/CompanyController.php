...
class CompanyController extends Controller
{
    ...
    public function edit_password_update(Request $request)
    {
        ...
        $request->validate([
            ...
            'password.regex' => 'Your new password must contain at least one special character.',
    ...
    public function application_status_change(Request $request)
    {
        ...
        $approvedCandidatesCount = CandidateApplication::where('job_id', $request->job_id)->where('status', 'Approved')->count();
        $job_single = Job::where('id',$request->job_id)->first();

        if($job_single->vacancy == $approvedCandidatesCount) {
            $job_single->active_apply = 1;
            $job_single->update();
        }else{
            $job_single->active_apply = 0;
            $job_single->update();
        }
